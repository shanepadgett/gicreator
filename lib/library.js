...
Gicreator.writeFile = reqTypes => {
  getTypes().then(availableTypes => {
    ...
    added.proj.forEach(item => {
      write(`https://raw.githubusercontent.com/github/gitignore/master/${item}.gitignore`, formatName(item))
    })

    added.global.forEach(item => {
      write(`https://raw.githubusercontent.com/github/gitignore/master/Global/${item}.gitignore`, formatName(item))
    })
